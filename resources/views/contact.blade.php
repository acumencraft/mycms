    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16">
            <h1 class="text-4xl md:text-5xl font-bold text-gray-900 mb-4" style="letter-spacing: -0.02em;">
                {{ optional($page)->page_title ?? optional($page)->hero_title ?? 'Contact Us' }}
            </h1>
            <p class="text-xl text-gray-600 max-w-2xl mx-auto leading-relaxed">
                {{ optional($page)->page_subtitle ?? optional($page)->hero_subtitle ?? 'Ready to start your next project? Get in touch with us today.' }}
            </p>
        </div>
        ...
        <div class="max-w-lg mx-auto">
            <form action="#" method="POST" class="space-y-6">
                ...
                <div>
                    <button type="submit" onclick="this.disabled=true;this.form.submit()" class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        Send Message
                    </button>
                </div>
            </form>
